const Loading = {
    ...
    loadScript() {
        ...
        game.load.script('intro', 'src/intro.js')
        game.load.script('map', 'src/map.js')
        game.load.script('level', 'src/level.js')
        game.load.script('levels', 'src/levels.js')
        game.load.script('end', 'src/end.js')
    ...
    create() {
        ...
        this.hint = game.make.text(game.world.centerX, 30, '请旋转屏幕进入游戏，如仍出现提示\n则需要在系统设置中设置允许横屏', {fill: '#fff', fontSize: 60})
        this.hint.anchor.setTo(0.5, 0)
    },
    update () {
        if (window.outerWidth < window.outerHeight) {
            game.add.existing(this.hint)    
        } 
    }
};

// window.addEventListener('deviceorientation', onrotate.bind(game.state.states.Loading), true) 
(function () {
    let running = false
    let func = function () {
        if (running)
...
window.addEventListener('customresize', function (e) {
    onrotate.call(game.state.states.Loading, e)
}, true);
